"""
Bulk ingest 50 ToS documents directly via OpenAI + Supabase REST APIs.
No app imports needed. Run from anywhere:
    python scripts/bulk_ingest_direct.py
"""
import json
import os
import sys
...
def embed_batch(texts: list[str]) -> list[list[float]]:
    payload = json.dumps({"input": texts, "model": EMBEDDING_MODEL}).encode()
...
def ingest(text: str, source: str, category: str) -> int:
...
def main() -> None:
    ...
    for i, doc in enumerate(DOCUMENTS, 1):
        try:
            n = ingest(doc["text"], source=doc["source"], category=doc["category"])
            print(f"[{i:02d}/50] {doc['source']} ({doc['category']}) → {n} chunk(s)")
            total_chunks += n
        except Exception as e:
            print(f"[{i:02d}/50] ERROR {doc['source']}: {e}", file=sys.stderr)
